...
- **v2.3.0** (Release date July 26, 2026)

  - Support Google Gemini, Anthropic, and OpenRouter for OCR alongside OpenAI. Select with `OCR_PROVIDER`/`OCR_MODEL`, or pass an `ocr_provider` to `convert_file` and the `initialize_*`/`convert_*` functions.
  - Add `gemini`, `anthropic`, and `all` optional dependency extras. OpenAI remains a core dependency and also serves OpenRouter.
  - FIX: Importing `ebook2text` no longer raises `KeyError` when API key environment variables are unset. Credentials are resolved lazily, on the first OCR call.
  - FIX: OCR responses were capped at 10 tokens, truncating nearly all extracted text. The cap is now 1000 tokens, configurable via `OCR_MAX_TOKENS`.
  - FIX: Image media type is detected from magic bytes rather than assumed to be PNG, and unrecognized formats are transcoded. JPEG images in EPUB and DOCX files previously sent a mismatched media type.
  - FIX: EPUB images referenced by relative path (`../Images/name.jpg`) are now resolved instead of raising `AttributeError`.
  - BREAKING: `ebook2text.ocr.run_ocr` is deprecated in favor of `get_ocr_provider().perform_ocr()`. The module-level `CLIENT`, `MODEL`, `GPT_REFUSALS`, `create_payload`, and `create_image_role_list` names are removed.
